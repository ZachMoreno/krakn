...
angular.module('krakn.controllers', [
  ...
  .controller('ChatCtrl', ['$scope', 'syncData', '$ionicScrollDelegate', '$ionicLoading', '$rootScope', '$location',
      function($scope, syncData, $ionicScrollDelegate, $ionicLoading, $rootScope, $location) {
         
         // split email on @ for minor privacy improvement before binding to $scope
         if (!$rootScope.auth.user.email) {
            $location.path('krakn/login');
         } else {
         ...
         $scope.userEmail2 = $rootScope.auth.user.email.split('@')[0];

         $scope.appliedClass = function(myObj) {
              if (myObj === $scope.userEmail2) {
                  console.log(myObj);
                  console.log($scope.userEmail2);
                  ...
                  return "isNotMe"; // Or even "", which won't add any additional classes to the element
              }
          }
